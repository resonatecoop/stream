...
class Player extends Nanocomponent {
  ...
  renderPlayer () {
    ...
    let infoBar = ''
    if (!isAuthenticated) {
      infoBar = html`<a class=${classesInfoBar} href="${process.env.APP_HOST}/api/v3/user/connect/resonate" target="_blank" rel="noopener noreferer">Log in to listen to full song</a>`
