  ...
  constructor (id, state, emit) {
    ...
    this.local.playback.on('playing', async () => {
      ...
      if (isNew) {
        ...
        await sound.load(this.local.src, { useFetch: process.env.NODE_ENV === 'development', token: this.state.token })
      }
  ...
  createElement (props = {}) {
    ...
    if (!this.local.track.id) {
      ...
      if (props.src !== null && props.src !== this.local.src) {
        ...
        sound.load(this.local.src, { useFetch: process.env.NODE_ENV === 'development', token: this.state.token })
      }
    }
